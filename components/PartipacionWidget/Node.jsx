...
class PartipacionWidgetNode extends React.Component {
  ...
  render() {
    ...
    if (y > 1) {
      y+= 30;
      text = <text textAnchor="left" x="30" y={`-${this.props.width / 2}`} dy=".35em" transform="rotate(90)">{this.props.name}</text>;
      color = "#000";
    }

    return (
      <g onMouseEnter={this.mouseEnter} className={cx(s.node, {[s.node__active]: this.props.active})} transform={`translate(${this.props.x},${y})`}>
